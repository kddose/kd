...
def configure_ngrok():
    ngrok_token = "2sCx2GVL4EDC2ckLArIpUNbaZfe_B5YzAm4a9vjwhhhoLurW"
    while True:
        try:
...
def update_soul_txt(public_url):
    with open("rdx3.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in rdx3.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("rdx3.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated rdx3.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update rdx3.txt on VPS: {str(e)}")
