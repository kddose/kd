...
def configure_ngrok():
    ngrok_token = "2sCvW5dlVglbjfMBOVF8sJgsEC5_7JNAPZGzqV4mqXD1nyBTe"
    while True:
        try:
...
def update_soul_txt(public_url):
    with open("rdx2.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in rdx2.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("rdx2.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated rdx2.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update rdx2.txt on VPS: {str(e)}")
