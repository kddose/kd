...
def configure_ngrok():
    ngrok_token = "2sCubGdow0llvnQG4q6fnEwhVxL_XCe5TbKabxSGD8DeWwfX"
    while True:
        try:
...
def update_soul_txt(public_url):
    with open("rdx1.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in rdx1.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("rdx1.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated rdx1.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update rdx1.txt on VPS: {str(e)}")
